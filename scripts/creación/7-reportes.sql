...
BEGIN
    ...
    OPEN cursorReporte FOR
        SELECT  
            ...
            ROUND((gastos/(gastos + (paquetes * precio)))*100,2) gastosgraf,
            ROUND(((paquetes * precio)/(gastos + (paquetes * precio)))*100,2) ingresosgraf,
            mes fechagraf
        FROM  
            (SELECT     
...
BEGIN
    OPEN cursorReporte FOR
        SELECT   
            to_char(pag.PAG_FECHA, 'MONTH YYYY') "Mes",   
            med.MED_NOMBRE nombregraf,   
            TO_NUMBER(LISTAGG(DISTINCT ROUND(subg.cant_medio*100/subg.cant_mes,2))) montograf  
        ...
        GROUP BY to_char(pag.PAG_FECHA, 'MONTH YYYY'),med.MED_NOMBRE,subg.cant_mes   
        ORDER BY to_char(pag.PAG_FECHA, 'MONTH YYYY');
END;
...
BEGIN
    OPEN cursorReporte FOR
        SELECT 
            to_char(pag.PAG_FECHA, 'MONTH YYYY') "Mes", 
            PAG.PAG_CANAL nombregraf, 
            TO_NUMBER(LISTAGG(DISTINCT ROUND(subg.cant_por_canal*100/subg.cant_mensual,2))) montograf
        ...
        WHERE to_char(pag.PAG_FECHA, 'MONTH YYYY') = to_char(fecha_mes, 'MONTH YYYY') OR fecha_mes IS NULL 
        group by to_char(pag.PAG_FECHA, 'MONTH YYYY'), PAG.PAG_CANAL, subg.cant_mensual
        ORDER BY to_char(pag.PAG_FECHA,'MONTH YYYY');
END;
