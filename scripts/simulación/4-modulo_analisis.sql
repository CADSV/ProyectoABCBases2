...
CREATE OR REPLACE PACKAGE BODY MODULO_ANALISIS AS
    ...
    BEGIN 
        ...
        LOOP 
            ...
            DBMS_OUTPUT.PUT_LINE('');
            UPDATE DISPONIBILIDAD SET dis_fecha.fecha_fin = SYSDATE WHERE ser_id = servicio_menos_demandado.ser_id;
        ...
        LOOP 
            ...
            DBMS_OUTPUT.PUT_LINE('');
            UPDATE DISPONIBILIDAD SET dis_fecha.fecha_fin = SYSDATE WHERE ser_id = sservicio_peor_calificado.ser_id;
            DBMS_OUTPUT.PUT_LINE('Se descontinuó el servicio.');
            DBMS_OUTPUT.PUT_LINE('');            
            cont := cont + 1; 
        END LOOP; 
        ...
        LOOP 
            ...
            DBMS_OUTPUT.PUT_LINE('');
            UPDATE DISPONIBILIDAD SET dis_fecha.fecha_fin = SYSDATE WHERE ser_id = servicio_peor_ganancia.ser_id;
            DBMS_OUTPUT.PUT_LINE('Se descontinuó el servicio.');
            DBMS_OUTPUT.PUT_LINE('');
            cont := cont + 1; 
        END LOOP;

        UPDATE DISPONIBILIDAD SET dis_fecha.fecha_fin = (dis_fecha.fecha_fin + 365) WHERE dis_fecha.fecha_fin > SYSDATE;
